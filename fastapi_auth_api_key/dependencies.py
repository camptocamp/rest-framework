...
def authenticated_auth_api_key(
    ...
    # TODO we need to filter out based on api groups that are linked to an endpoint
    # Can we have a reference to current endpoint here?
    # if auth_api_key not in endpoint.auth_api_key_group_id.api_key_ids:
